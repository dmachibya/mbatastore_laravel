    ...
    function get_payment_config_info($code, $shop = null)
    {
        ...
        switch ($code) {
            ...
            case 'bkash':
                ...
                return [
                    'config' => $config,
                    'msg' => trans('packages.bkash.pay_with_bkash'),
                ];
            ...
            case 'twoCheckout':
                if ($shop) {
                    $config = $shop->config->twoCheckout ?? null;
                } else {
                    $config = config('twoCheckout.api');
                }

                return [
                    'config' => $config,
                    'msg' => trans('packages.twoCheckout.pay_with_twoCheckout'),
                ];

            case 'mtnMoney':
                if ($shop) {
                    $config = $shop->config->mtnMoney ?? null;
                } else {
                    $config = config('mtnMoney.api');
                }

                return [
                    'config' => $config,
                    'msg' => trans('packages.mtnMoney.pay_with_mtnMoney'),
                ];

            case 'upiPayment':
                $config = $shop ? $shop->config->upiPayment : config('upiPayment.api');

                return [
                    'config' => $config,
                    'msg' => trans('packages.upiPayment.pay_with_upiPayment'),
                ];
        ...
        switch ($attribute) {
            ...
            case 'status':
                ...
                if (class_basename($model) == 'Dispute') {
                    $old = get_dispute_status_name($old);
                    $new = get_dispute_status_name($new);
                }
                break;
...
if (!function_exists('prepareFilteredListings')) {
     ...
     * @param \Illuminate\Support\Collection $catSubGroup category subgroup items
     *
...
if (!function_exists('crosscheckAndUpdateOldCartInfo')) {
    ...
    function crosscheckAndUpdateOldCartInfo($request, Cart $cart)
    {
        ...
        foreach ($cart->inventories as $item) {
            ...
            $unit_price = is_incevio_package_loaded('wholesale')
                ? get_wholesale_unit_price($item, $temp_qtt)
...
if (!function_exists('is_catalog_enabled')) {
    /**
     * Check if the catalog system enabled
     */
